...
/**
 * Quota Status module (deep, ports & adapters): everything behind the footer slot,
 * /antigravity usage, and /antigravity settings — Quota Pool wire parse, urgent-window
 * math, ratio self-calibration, throttle, cache, provider settings UI, and local
 * config persistence.
 * Callers use footerFor / ingest / refresh / refreshAndPaint / openSettings only;
 * the fetch, the ratio state, and the file shape behind the injected store are internal.
 */
...
export const FOOTER_MODE_NOTES: Readonly<Record<string, string>> = Object.freeze({
  smart: FOOTER_MODES.smart.note!,
  all: FOOTER_MODES.all.note!,
});
...
function writeProviderConfigAtomically(file: string, data: ProviderFileConfig): boolean {
...
// preserves them.
function updateProviderConfig(file: string, mutate: (config: ProviderFileConfig) => void): boolean {
  try {
...
export function fileQuotaStatusStore(file = defaultConfigFile()): QuotaStatusStore {
  return {
    ...
    saveQuotaState: (state) =>
      updateProviderConfig(file, (config) => {
        const states =
          config.states && typeof config.states === "object" && !Array.isArray(config.states)
            ? config.states
            : {};
        const quota =
          states.quota && typeof states.quota === "object" && !Array.isArray(states.quota)
            ? states.quota
            : {};
...
export class QuotaStatusCoordinator {
  ...
  // Opens provider settings dialog/menu and updates state atomically.
  async openSettings(ctx: ExtensionCommandContext): Promise<void> {
    await openSettings(ctx, this);
  }
