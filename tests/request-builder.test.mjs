...
const fixtureTurn2 = JSON.parse(
  fs.readFileSync(newestCapture("stream_turn2_toolresult.req.json"), "utf-8")
);
const fixtureTurn4 = JSON.parse(
  fs.readFileSync(newestCapture("stream_turn4_thinking.req.json"), "utf-8")
);
const fixtureTurn5 = JSON.parse(
  fs.readFileSync(newestCapture("stream_turn5_multiturn.req.json"), "utf-8")
);
const fixtureTurn6 = JSON.parse(
  fs.readFileSync(newestCapture("stream_turn6_toolerror.req.json"), "utf-8")
);
...
test("Seam 1: a gpt reasoning turn replays as text only (gpt turn12 → turn13)", () => {
  // The frozen gpt-oss turn's response carried 15 thought parts:
...
test("Seam 1: trajectory is a per-conversation v5 UUID and the numeric sessionId is the agy wire constant", () => {
  ...
  // An explicit numeric sessionId (stream.ts override) wins verbatim
...
test("Seam 1: tool conversion formats legacy parameters for Gemini (agy parity, never parametersJsonSchema)", () => {
  ...
  // (wire schema carries zero parametersJsonSchema declarations).
...
test("Seam 1: buildAntigravityRequestBody ignores foreign JSON thoughtSignatures (OpenAI Responses) and converts thinking to plain text", () => {
  ...
  // The wire key is camelCase `thoughtSignature`; the snake_case spelling never appears.
  ...
  // Tool call does not carry the foreign signature — it gets the Gemini sentinel instead.
  assert.equal(modelTurn.parts[1].functionCall.name, "bash");
  assert.equal(modelTurn.parts[1].thoughtSignature, SKIP_THOUGHT_SIGNATURE_VALIDATOR);
});

// --- Cross-provider model switch inside an open tool turn (live probe 2026-09-10) ---
//
// Session evidence (opencode-go/muse-spark-1.3-contributor turn between two antigravity/
// gemini-3.8-flash turns, model switched while a tool turn was open): the foreign turn's
// functionCall carries no thoughtSignature at all — the other provider never issues one —
// and replaying it as the pending step of the current turn is what the backend rejected:
//
//   400 INVALID_ARGUMENT "Function call is missing a thought_signature in functionCall
//   parts. ... function call `default_api:bash` , position 32"
//
// Google's rule (ai.google.dev/gemini-api/docs/thought-signatures) is that validation
// covers "all steps in the current turn", and "a turn begins with the most recent user
// message that is not a functionResponse" — hence the 400 only while the foreign tool turn
// was current, and the same unsigned part passing once a new user message moved the
// boundary. The documented remedy for a functionCall "not generated by the API" is the
// skip_thought_signature_validator sentinel, which the builder now emits for Gemini
// requests. Live probe validation (gemini-3.8-flash-low): unsigned pending functionCall → 400 with the message above; the
// identical body with the sentinel on that single part → 200.
const GEMINI_FAMILY_SIGNATURE = "EtUOCtIOARFNMg8lE2aQ3yiigw==";
const FOREIGN_TOOL_CALL_ID =
  "call_01a08963aae8763196b0609d8bb79af6|fc_01a08963aae8763196b0609d8bb79af6";

const CROSS_PROVIDER_TOOL_TURN = [
  { role: "user", content: "clear the tmux status-right" },
  {
    role: "assistant",
    ...
    content: [
      {
        ...
        id: "call_437029",
        name: "bash",
        arguments: { command: "tmux set -g status-right ''" },
        thoughtSignature: GEMINI_FAMILY_SIGNATURE,
      },
    ],
  },
  { role: "toolResult", toolCallId: "call_437029", toolName: "bash", content: "ok" },
  {
    role: "assistant",
    ...
    content: [
      {
        type: "toolCall",
        id: FOREIGN_TOOL_CALL_ID,
        name: "bash",
        arguments: { command: "tmux show-options -g status-right" },
      },
    ],
  },
  {
    role: "toolResult",
    toolCallId: FOREIGN_TOOL_CALL_ID,
    toolName: "bash",
    content: "status-right ''",
  },
];

const buildCrossProviderTurn = (messages) =>
...
// The builder emits toolResults as model turns carrying functionResponse (agy parity),
// so the current turn always starts after the last user turn.
function currentTurn(contents) {
  let start = 0;
  contents.forEach((c, i) => {
    if (c.role === "user") start = i + 1;
  });
  return contents.slice(start);
}

// Mirrors the backend validator for the documented rule: every functionCall in the
// current turn must carry a thoughtSignature.
function violatesThoughtSignatureRule(contents) {
  return currentTurn(contents).some((turn) =>
    turn.parts.some((p) => p.functionCall && !p.thoughtSignature)
  );
}

test("Seam 1: cross-provider tool turn at the tip carries the documented sentinel (live probe shape)", () => {
  const contents = buildCrossProviderTurn(CROSS_PROVIDER_TOOL_TURN).request.contents;

  // The same-provider Gemini turn keeps its real signature; the foreign turn has none to
  // replay, so it gets the sentinel — the shape the probe proved the backend accepts.
  assert.equal(contents[1].parts[0].thoughtSignature, GEMINI_FAMILY_SIGNATURE);
  assert.equal(contents[3].parts[0].functionCall.name, "bash");
  assert.equal(contents[3].parts[0].thoughtSignature, SKIP_THOUGHT_SIGNATURE_VALIDATOR);
  assert.equal(violatesThoughtSignatureRule(contents), false);

  // The sentinel is a last resort for unsigned calls only: exactly one part carries it.
  const sentinels = contents
    .flatMap((turn) => turn.parts)
    .filter((p) => p.thoughtSignature === SKIP_THOUGHT_SIGNATURE_VALIDATOR);
  assert.equal(sentinels.length, 1);
});

test("Seam 1: sentinel is Gemini-scoped, foreign-only and independent of the turn boundary", () => {
  // Position: one user message later the foreign call is replayed as context (no longer the
  // pending step), and it still carries the sentinel — the fix does not depend on where the
  // turn boundary happens to sit.
  const laterTurn = buildCrossProviderTurn([
    ...CROSS_PROVIDER_TOOL_TURN,
    { role: "user", content: "go on" },
  ]).request.contents;
  assert.equal(laterTurn[laterTurn.length - 1].parts[0].text, "go on");
  assert.equal(violatesThoughtSignatureRule(laterTurn), false);
  assert.equal(
    laterTurn
      .flatMap((turn) => turn.parts)
  ...
  // Scope: Claude requests keep today's shape — no capture shows their wire accepting the
    ...
    context: {
      messages: [
        { role: "user", content: "clear the tmux status-right" },
        {
          ...
          model: "gemini-3.8-flash",
          content: [
            { type: "toolCall", id: "call_437030", name: "bash", arguments: { command: "true" } },
          ],
        },
        { role: "toolResult", toolCallId: "call_437030", toolName: "bash", content: "ok" },
      ],
    },
  }).request.contents;
