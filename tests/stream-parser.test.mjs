...
test("Seam 2: parseAntigravitySseChunks extracts thinking and thoughtSignature from Turn 1", () => {
  const result = parseAntigravitySseChunks(sseTurn1);
...
test("Seam 2: parseAntigravitySseChunks extracts usage metadata", () => {
  const result = parseAntigravitySseChunks(sseTurn1);
...
test("Seam 2: parseAntigravitySseChunks handles text responses from Turn 4", () => {
  const sseTurn5 = fs.readFileSync("captures/agy_cli_1.1.26/stream_turn5_multiturn.resp.sse", "utf-8");
  const result = parseAntigravitySseChunks(sseTurn5);
