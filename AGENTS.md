...
- **Capture Before Changing Protocol**: When touching endpoints, headers, `functionCall`/`functionResponse` shapes, error payloads, or image parts, verify against the official `agy` CLI via `mitmproxy` first — never speculate. Uncovered cases (e.g. error tool results, image results) need a fresh targeted capture. Runbook: `captures/README.md`. Skill: `wire-fingerprint`.
