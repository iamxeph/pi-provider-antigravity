...
- **Multi-Turn Live Verification (10-Turn Parity Test)**: When `agy` behavior is ambiguous, or after touching request building, session management, or stream translation, verify live wire equivalence across a 10-turn session (`mitmdump -p 18080`) covering conversational turns, tool success, tool failure, follow-up reasoning, and final summary — the canonical scenario is `captures/scenarios.json` (plan: `npm run capture:plan`; runbook: `captures/README.md` §2). Diff `v1internal:streamGenerateContent` flows field-by-field (headers, envelope, labels/`request_id` sequence, session invariants, `functionResponse` `{ output }` envelope, thought-signature replay).
  ...
  - **pi-side live run (verifying this provider, not `agy`)**: `npm run build` first — pi loads `dist/index.js` through the `settings.json` packages entry, so a stale bundle silently verifies the old code. Drive the real client through the same proxy: `NODE_USE_ENV_PROXY=1 HTTPS_PROXY=http://127.0.0.1:18080 SSL_CERT_FILE=~/.mitmproxy/mitmproxy-ca-cert.pem pi-unwrapped -p "<prompt>" --model antigravity/gemini-3.8-flash --thinking high` (Node's `fetch` ignores the proxy env without `NODE_USE_ENV_PROXY`). For a multi-turn chain, `--session-id <id>` creates a session and `--session <id>` appends to it — `-c/--continue` cannot be combined with `--session-id` (pi 0.85.1), so use that explicit pair. `-p` also runs the `/antigravity usage|models|refresh` subcommands and executes tools without prompting, which makes the quota and catalog paths cheap to check. Then diff the captured requests against the newest `captures/agy_cli_<version>/` shapes: envelope keys, label keys, the sessionId constant, the User-Agent row, the step counters, and the replayed turn's part shape (part-split thinking, lone-signature text, gpt text-only). Transport-level header differences (lowercased names, undici's extra headers) are expected by decision — parity is enforced at the header object, and the measured cost of closing that gap is in ADR-0011.
